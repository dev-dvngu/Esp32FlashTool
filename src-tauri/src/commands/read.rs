...
pub fn read_flash(app: AppHandle, payload: ReadFlashPayload) -> Result<(), String> {
    {
        ...
        let guard = state.child.lock().map_err(|_| "Lock failed".to_string())?;
        if guard.is_some() { return Err("Đang bận...".into()); }
    }
...
fn read_worker(
    ...
    emit(&app, "status_change", &StatusChangeEvent { status: "reading".into() });

    let read_args: Vec<String> = vec![
        "--port".into(), payload.port, "--baud".into(), payload.baud.to_string(),
        "read-flash".into(), // v5.x dùng read-flash
        payload.offset, payload.size, payload.output_file,
    ];
    ...
    read_cmd.args(&read_args).stdout(Stdio::piped()).stderr(Stdio::piped());
    #[cfg(target_os = "windows")]
    read_cmd.creation_flags(0x08000000);

    let mut child = read_cmd.spawn().map_err(|e| format!("Spawn error: {e}"))?;
                ...
                for &byte in &buffer[..n] {
                    if byte == b'\n' || byte == b'\r' || byte == 0x08 {
                        if !line_buffer.is_empty() {
                            ...
                            if byte == b'\n' { emit(&app_out, "log_line", &LogLineEvent { text: line.clone(), level: "info".into() }); }
    ...
    let t_err = if let Some(err) = stderr {
        Some(std::thread::spawn(move || {
            for line in BufReader::new(err).lines().flatten() {
                emit(&app_err, "log_line", &LogLineEvent { text: line, level: "error".into() });
            }
        }))
    } else { None };
